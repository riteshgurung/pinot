...
public class ValueInTransformFunction extends BaseTransformFunction {
  ...
  public void init(@Nonnull TransformExpressionTree expression, @Nonnull Map<String, DataSource> dataSourceMap) {
    ...
    if (numArguments < 2) {
      throw new BadQueryRequestException("At least 2 arguments are required for VALUE_IN transform function");
    }

    // Check that the first argument is a column or a transform function
    TransformExpressionTree mainExpression = arguments.get(0);
    TransformExpressionTree.ExpressionType mainExpressionType = mainExpression.getExpressionType();
    if (mainExpressionType != TransformExpressionTree.ExpressionType.IDENTIFIER
        && mainExpressionType != TransformExpressionTree.ExpressionType.FUNCTION) {
      throw new BadQueryRequestException(
          "The first argument of VALUE_IN transform function must be a column or a transform function");
    }

    // Check that the first argument is a multi-valued column or transform function
    _mainTransformFunction = TransformFunctionFactory.get(mainExpression, dataSourceMap);
    DataSourceMetadata metadata = _mainTransformFunction.getResultMetadata();
    if (metadata.isSingleValue()) {
      throw new BadQueryRequestException(
          "The first argument of VALUE_IN transform function must be a multi-valued column or transform function");
    }

    // Collect all values for the VALUE_IN transform function
    _stringValueSet = new HashSet<>(numArguments - 1);
    for (int i = 1; i < numArguments; i++) {
      TransformExpressionTree inValue = arguments.get(i);
      if (inValue.getExpressionType() != TransformExpressionTree.ExpressionType.LITERAL) {
        throw new BadQueryRequestException(
            "All the arguments of VALUE_IN transform function except the first one must be values");
      }
      _stringValueSet.add(inValue.getValue());
    }
  }
