...
public class SubtractionTransformFunction extends BaseTransformFunction {
  ...
  private final double[] _differences = new double[DocIdSetPlanNode.MAX_DOC_PER_CALL];
  ...
  public void init(@Nonnull TransformExpressionTree expression, @Nonnull Map<String, DataSource> dataSourceMap) {
    ...
    if (arguments.size() != 2) {
      throw new BadQueryRequestException("Exactly 2 arguments are required for SUB transform function");
    }
