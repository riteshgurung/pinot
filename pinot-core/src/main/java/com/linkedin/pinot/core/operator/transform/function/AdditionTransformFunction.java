...
public class AdditionTransformFunction extends BaseTransformFunction {
  ...
  private final double[] _sums = new double[DocIdSetPlanNode.MAX_DOC_PER_CALL];
  ...
  public void init(@Nonnull TransformExpressionTree expression, @Nonnull Map<String, DataSource> dataSourceMap) {
    ...
    if (numArguments < 2) {
      throw new BadQueryRequestException("At least 2 arguments are required for ADD transform function");
    }
    ...
    for (TransformExpressionTree argument : arguments) {
      if (argument.getExpressionType() == TransformExpressionTree.ExpressionType.LITERAL) {
        _literalSum += Double.parseDouble(argument.getValue());
      } else {
